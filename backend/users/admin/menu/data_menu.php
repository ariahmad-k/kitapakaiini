                            ...
                            <table id="datatablesSimple">
                                ...
                                <tbody>
                                    <?php while ($row = mysqli_fetch_assoc($result)) { 
                                        $isActive = ($row['status_produk'] === 'aktif'); // true kalau aktif
                                        $isInactive = ($row['status_produk'] === 'tidak aktif'); // true kalau nonaktif ?>
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <!-- Tombol Hapus disabled jika produk non aktif -->
                                                <?php if ($isInactive) : ?>
                                                    <button class="btn btn-danger btn-sm" title="Hapus (nonaktif)" disabled style="cursor:not-allowed; opacity:0.5;"> <i class="fas fa-trash"></i></button>
                                                <?php else: ?>
                                                ...
                                                <?php endif; ?>
                                                <!-- Tombol Aktifkan disabled jika produk sudah aktif -->
                                                <?php if ($isActive) : ?>
                                                    <button class="btn btn-success btn-sm" title="Sudah aktif" disabled style="cursor:not-allowed; opacity:0.5;"> Aktifkan</button>
                                                <?php else: ?>
                                                    <a href="menu_aktif.php?id_produk=<?= urlencode($row['id_produk']) ?>" class="btn btn-success btn-sm" onclick="return confirm('Aktifkan produk ini?');" title="Aktifkan">Aktifkan</a>
                                                <?php endif; ?>
                                            </td>
                                        </tr>
                                    <?php } ?>
